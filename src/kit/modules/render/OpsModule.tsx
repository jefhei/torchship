/**
 * M2-T4 — the authored ops module as an R3F component.
 *
 ...
 * from `moduleParts(OPS_MODULE)` (the M2-T1 builders, rule 8), never from
 * hand-placed meshes.
 *
 ...
 * src/kit/modules/ops.test.ts (see references/testing-r3f-in-jsdom.md).
 */
...
/** The ops module: airlock + suit locker, tool wall, workbench, med bay. */
export function OpsModule({ position, rotation }: PrimitivePlacement) {
  return (
    <group {...placementProps({ position, rotation })}>
      <KitParts parts={moduleParts(OPS_MODULE)} />
    </group>
  )
}
