/**
 * M2-T5 — the authored engineering module as an R3F component.
 *
 ...
 * from `moduleParts(ENGINEERING_MODULE)` (the M2-T1 builders, rule 8), never
 * from hand-placed meshes.
 *
 ...
 * src/kit/modules/engineering.test.ts (see references/testing-r3f-in-jsdom.md).
 */
...
/** The engineering module: reactor bulkhead, drive glow window, placard. */
export function EngineeringModule({ position, rotation }: PrimitivePlacement) {
  return (
    <group {...placementProps({ position, rotation })}>
      <KitParts parts={moduleParts(ENGINEERING_MODULE)} />
    </group>
  )
}
